...
//ação para listar todos os médicos cadastrados no sistema, onde será necessário acessar o array de médicos do banco de dados e exibir os dados dos médicos de forma organizada.
  static listar() {
    try {
      if (database.length === 0) {
        return console.error("Nenhum médico encontrado."); //verifica se o array de médicos está vazio, caso esteja, exibe uma mensagem de erro indicando que nenhum médico foi encontrado.
      }
  ...
  static atualizar(id, novo_nome, novo_telefone, novo_email, novo_especialidade) {
    try {
        if (!id) {
      ...
      const medico = database.find((m) => m.id === id); //procura um médico no array de médicos do banco de dados com base no ID fornecido como parâmetro, utilizando o método find para localizar o médico correspondente. //Se o médico for encontrado, ele é armazenado na variável medico para que possa ser atualizado posteriormente.
      if (!medico) {
  ...
  static excluirPorId(id) {
    try {
        const medico = database.findIndex((m) => m.id === id); //procura o índice de um médico no array de médicos do banco de dados com base no ID fornecido como parâmetro, utilizando o método findIndex para localizar o índice do médico correspondente. //Se o médico for encontrado, o índice é armazenado na variável medico para que possa ser excluído posteriormente.
      if (medico === -1) {
